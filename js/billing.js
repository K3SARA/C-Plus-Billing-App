class Billing {
  ...
  getUnitOptions(selectedUnit = 'pcs') {
    const units = [
      ...
      el.innerHTML = `
        <div class="item-name-row">
          <div class="product-input-wrap">
            ...
            <input type="text" id="product-name-${item.id}" class="input-field product-name-input" placeholder="Search item or service" value="${this.escapeAttribute(item.name)}" style="font-size:1.1rem; font-weight:500;">
        ...
        <div class="item-controls-grid">
          ...
          <div class="item-field-block ${this.isCollectingOrderMode() ? 'hidden' : ''}">
            ...
            <select class="input-field item-unit-select" onchange="billing.updateItem('${item.id}', 'unit', this.value)">
              ${this.getUnitOptions(item.unit || 'pcs')}
            </select>
          </div>
        </div>
        <div class="item-field-block ${this.isCollectingOrderMode() ? 'hidden' : ''}">

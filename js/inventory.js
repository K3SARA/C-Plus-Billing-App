class Inventory {
  ...
  renderExistingProductOptions() {
    ...
    select.innerHTML = '<option value="">New item or choose existing</option>';
...
<body>
  ...
  async printInventoryReport() {
    ...
    window.pdfDownload.downloadStructuredPdf('Inventory Report', {
      ...
      tables: [{
        headers: ['Item', 'Added Qty', 'Selling Price', 'Cost Price', 'Added Date & Time'],
        rows: logs.map((log) => [
          log.productName || '-',
    ...
    this.products.forEach((p) => {
      ...
      el.innerHTML = `
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-top:14px;">
          ...
          <div style="display:flex; flex-direction:column; gap:6px;">
            <span style="color:var(--text-muted); font-size:0.85rem;">Cost Price</span>

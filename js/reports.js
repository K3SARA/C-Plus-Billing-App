class ReportsView {
  ...
  renderCollectingOrdersReport(range) {
      ...
      <tr>
        <td class="collecting-order-no">
          ...
          <div class="collecting-row-actions no-pdf">
            <button type="button" class="btn-tool collecting-row-print-btn" onclick="reportsView.printCollectingOrderA4(${Number(order.id)})" aria-label="Print A4" title="Print A4">${this.getPrintIcon()}</button>
        ...
        <td class="collecting-actions no-pdf">
          <button type="button" class="btn-tool collecting-print-btn" onclick="reportsView.printCollectingOrderA4(${Number(order.id)})">Print A4</button>
    ...
    return `
      <div class="report-title-row">
        ...
        <button class="btn-tool report-pdf-btn" onclick="reportsView.printCollectingOrdersA4()">Print All A4</button>
      </div>
      ${this.metricGrid([
  ...
  printCollectingOrderA4(orderId) {
    const order = this.collectingOrders.find((item) => Number(item.id) === Number(orderId));
    if (!order) {
      alert('Collecting order was not found.');
      return;
  ...
  buildCollectingOrdersA4Html(orders) {
    ...
    const slips = orders.map((order) => {
      ...
      const itemsRows = (order.items || []).map((item) => `
        <tr>
          <td>${esc(item.name || '')}</td>
          <td class="qty">${esc(formatQtyLine(item))}</td>
          <td class="num">-</td>
          <td class="num">-</td>
        </tr>
      `).join('');
      ...
      return `
        <section class="invoice-page receipt">
          ...
          <div class="invoice-content">
            ...
            <section class="invoice-party-grid">
              <div>
                ...
                <p class="customer-name">${esc(order.customerName || 'Walk-in Customer')}</p>
                <div class="customer-lines">${customerLines}</div>
              </div>
              <table class="meta-table">
                <tr><th>Order#</th><td>${esc(order.orderNumber || order.id || '-')}</td></tr>
                <tr><th>Date</th><td>${esc(dateObj.toLocaleDateString())}</td></tr>
              </table>
            </section>
          ...
          <footer class="invoice-footer">
            <div class="footer-item"><span class="footer-icon">T</span><span>${esc(business.businessPhone || '+94 75 340 6070')}</span></div>
            <div class="footer-item"><span class="footer-icon">A</span><span>${esc(business.businessAddress || 'Ja-ela')}</span></div>
          </footer>
        </section>
      `;
    }).join('');
...
<title>Collecting Orders A4</title>
<style>${window.share?.getA4PrintStyles ? window.share.getA4PrintStyles() : ''}</style>
</head>
<body>
  ...
  <\/script>
  ${slips || '<section class="invoice-page"><div class="invoice-content">No collecting orders</div></section>'}
</body>
</html>`;
  }

  printCollectingOrdersA4() {
    ...
    printWindow.document.write(this.buildCollectingOrdersA4Html(orders));
  ...
  renderDayCloseReport(range, rangeBills) {
    ...
    return `
      <div class="report-title-row">
        ...
        <button class="btn-tool report-pdf-btn" onclick="reportsView.printDayCloseA4()">Print A4</button>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Day Close A4</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;500;600;700;800&display=swap');
  @page { size: A4; margin: 0; }
  * { box-sizing: border-box; }
  html, body { margin: 0; min-height: 100%; background: #edf2f7; }
  body { color: #000; font-family: "Open Sans", Arial, sans-serif; font-size: 13px; font-weight: 700; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  .screen-actions { position: sticky; top: 0; z-index: 20; background: #fff; padding: 10px; display: flex; gap: 8px; border-bottom: 1px solid #ddd; }
  ...
  .title { font-size: 38px; font-weight: 400; letter-spacing: 0.08em; margin: 0 0 4px; text-transform: uppercase; }
  .sub { color: #555; font-size: 13px; margin-bottom: 2px; }
  .rule { border-top: 1.5px solid #202020; margin: 14px 0; }
  .section-title {
    font-size: 18px;
    line-height: 1.15;
    margin: 12px 0 8px;
  ...
  .row { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 16px; padding: 4px 0; }
  .row span:last-child { text-align: right; }
  .block { padding: 7px 0; border-bottom: 1px solid #d4d4d4; break-inside: avoid; page-break-inside: avoid; }
  .block:last-child { border-bottom: 0; }
  .empty { padding: 4px 0; font-size: 12px; }
  @media screen { body { padding: 18px; } }
  @media (max-width: 820px) {
    body { padding: 0; background: #fff; }
    .sheet { width: 100%; min-height: 100vh; padding: 80px 18px 64px; box-shadow: none; }
  }
  @media print {
    html, body { width: 210mm; min-height: 297mm; padding: 0 !important; background: #fff !important; }
    .screen-actions { display: none !important; }
    .sheet { width: 210mm !important; min-height: 297mm !important; margin: 0 !important; box-shadow: none !important; }
  }
</style>
</head>
<body>
  ...
  printDayCloseA4() {
    const data = this.getDayCloseA4Data(this.getRange());
    const printWindow = window.open('', '_blank');
    if (!printWindow) {
      alert('Popup blocked. Please allow popups to print.');
      return;
    ...
    printWindow.document.open();
    printWindow.document.write(this.buildDayCloseA4Html(data));
    printWindow.document.close();
  }
